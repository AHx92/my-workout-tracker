...
class OfflineManager {
    ...
    if (this.isOnline) {
      setTimeout(async () => {
        try {
          ...
          console.log('[Offline] Sync on startup failed (expected on first run):', error);
        }
      }, 1000);
    }
  }
  ...
  getPendingWorkouts() {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        // Filter for unsynced workouts
        const allWorkouts = request.result || [];
        const pending = allWorkouts.filter(workout => workout.synced === false);
        resolve(pending);
      };
      request.onerror = () => reject(request.error);
    });
