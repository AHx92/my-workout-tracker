...
function addExercise() {
    ...
    // Add event listener for weight unit changes
...
async function loadExerciseDatabase() {
    try {
        const response = await fetch(`${API_URL}?action=getExerciseDatabase`);
        const data = await response.json();
        
        if (data.exercises) {
            await offlineManager.cacheExercises(data.exercises);
            populateExerciseLists(data.exercises);
        }
    } catch (error) {
